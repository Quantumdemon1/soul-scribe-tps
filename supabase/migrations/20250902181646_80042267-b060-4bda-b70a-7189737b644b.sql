...
-- Policies
-- Allow users to insert their own logs or anonymous logs (user_id is null)
CREATE POLICY IF NOT EXISTS "Users can insert own or anonymous error logs"
...
CREATE POLICY IF NOT EXISTS "Users can view their own error logs"
ON public.error_logs
FOR SELECT
TO authenticated
